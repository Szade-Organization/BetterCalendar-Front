...
import { changeEmail, changePassword, logoutUser } from '../../services/AuthService';
...
const changePasswordValidationSchema = Yup.object().shape({
    old_password: Yup.string().min(8, 'Password must have at least 8 characters').required('Password is required'),
...
const SettingsComponents = () => {
    ...
    const logoutMutation = useMutation({
        ...
        onError: () => {
            toast(<Toast type={ToastType.ERROR} message={"Something went wrong, try again"} />);
        },
    });

    const changePasswordMutation = useMutation({
        mutationFn: (changePasswordRequest) => changePassword(changePasswordRequest),      
    });
    ...
    return (
        <div className='flex justify-between gap-5 p-3'>
                    ...
                    {({ isSubmitting }) => (
                        <Form className='flex flex-col gap-5'>
                            <div className='flex flex-col gap-1'>
                                <label htmlFor="old_password" className='self-start'>Old password</label>
                                <Input type="password" name="old_password" />
                                <ErrorMessage name="old_password" component="div" className='text-red-500 font-semibold' />
                            </div>

                            <div className='flex flex-col gap-1'>
                                <label htmlFor="password" className='self-start'>New password</label>
                                <Input type="password" name="password" />
                                <ErrorMessage name="password" component="div" className='text-red-500 font-semibold' />
                            </div>

                            <div className='flex flex-col gap-1'>
                                <label htmlFor="password_confirm" className='self-start'>Confirm password</label>
                                <Input type="password" name="password_confirm" />
                                <ErrorMessage name="password_confirm" component="div" className='text-red-500 font-semibold' />
                            </div>
            ...
            <div className='flex flex-col justify-start'>
                <Button className='bg-red-400' onClick={handleLogout} disabled={logoutMutation.isLoading}>
