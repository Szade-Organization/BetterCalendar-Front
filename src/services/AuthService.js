...
export const changePassword = async (changePasswordRequest) => {
    try {
        const token = localStorage.getItem('jwt');
        const response = await axios.post(`${API_URL}/auth/change-password/`, changePasswordRequest, {
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Token ${token}`,
            },
...
export const changeEmail = async (changeEmailRequest) => {
    try {
        const token = localStorage.getItem('jwt');
        const response = await axios.post(`${API_URL}/auth/change-email/`, changeEmailRequest, {
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Token ${token}`,
            },
